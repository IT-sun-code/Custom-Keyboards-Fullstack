...
const BasketService = {
  ...
  increaseOne: async (cardId) => {
    const userId = localStorageService.getUserId();
    const { data } = await httpService.patch(basketEndpoint + "add/" + cardId, {
      userId,
    });
    return data;
  },
  ...
  delete: async (cardId) => {
    const userId = localStorageService.getUserId();
    const { data } = await httpService.delete(basketEndpoint + cardId, {
      params: { userId },
    });
    return data;
  },
  ...
  createBasket: async (cardId) => {
    const userId = localStorageService.getUserId();
    const { data } = await httpService.post(basketEndpoint + cardId, {
      userId,
    });
    return data;
  },
